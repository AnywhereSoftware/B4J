...
import anywheresoftware.b4a.objects.WorkbookWrapper.AbsSheetWrapper.SheetWrapper;
import anywheresoftware.b4a.objects.WorkbookWrapper.AbsSheetWrapper.WritableSheetWrapper;
...
public class WorkbookWrapper extends AbsObjectWrapper<Workbook>{
	...
	static WorkbookSettings getDefaultSettings(String encoding) {
		...
		ws.setTemporaryFileDuringWriteDirectory(new java.io.File(File.getDirTemp()));
		return ws;
	}
	...
	public static class WritableWorkbookWrapper extends AbsObjectWrapper<WritableWorkbook> {
		...
		public void Initialize2(String Dir, String FileName, Workbook Workbook) throws IOException {
			OutputStream out = File.OpenOutput(Dir, FileName, false).getObject();
			WritableWorkbook ww = jxl.Workbook.createWorkbook(out, Workbook, getDefaultSettings(Encoding));
			setObject(ww);
		}
		/**
		 * Returns the number of sheets.
	...
	@Hide
	@Events(values={"ValuesAvailable (Success As Boolean, Rows As List)"})
	public static class AbsSheetWrapper<T extends Sheet> extends AbsObjectWrapper<T> {
		 ...
		 */
		public void GetAllAsync(final BA ba, String EventName) {
			BA.runAsync(ba, this, EventName + "_valuesavailable", 
				new Object[] {false, null}, new Callable<Object[]>() {
					...
					public Object[] call() throws Exception {
						...
						for (int i = 0;i < rowCount;i++) {
							...
							Cell[] cells = getObject().getRow(i);
							for (int c = 0;c < row.length;c++) {
								if (c >= cells.length)
									row[c] = "";
								else
									row[c] = cells[c].getContents();
							}
							l1.Add(row);
						}
						return new Object[] {true, l1};
					}
